...
void keyPressCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	case(4): {//Jon's Model		
		if (state_W == GLFW_PRESS)
			jonArm.setTranslateModel(glm::vec3(jonArm.getTranslateModel().x, (jonArm.getTranslateModel().y + .005f), jonArm.getTranslateModel().z)); break;
		else if (state_S == GLFW_PRESS)
			jonArm.setTranslateModel(glm::vec3(jonArm.getTranslateModel().x, (jonArm.getTranslateModel().y - .005f), jonArm.getTranslateModel().z)); break;
		else if ((state_D == GLFW_PRESS) && mods == GLFW_MOD_SHIFT)
			jonArm.setTranslateModel(glm::vec3((jonArm.getTranslateModel().x - .005f), jonArm.getTranslateModel().y, jonArm.getTranslateModel().z)); break;
		else if ((state_A == GLFW_PRESS) && mods == GLFW_MOD_SHIFT)
			jonArm.setTranslateModel(glm::vec3((jonArm.getTranslateModel().x + .005f), jonArm.getTranslateModel().y, jonArm.getTranslateModel().z)); break;
		else if ((state_A == GLFW_PRESS) && mods != GLFW_MOD_SHIFT)
			switch (selectJoint) {
